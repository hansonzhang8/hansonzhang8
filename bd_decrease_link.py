...
def sqrtPriceX96_to_price(sqrt_price_x96: float) -> float:
    """Convert sqrtPriceX96 to token ratio (link per ETH)."""
...
def step_1_load_and_merge(liquidations_csv: str, link_slot0_csv: str, eth_slot0_csv: str) -> pd.DataFrame:
    ...
    # Load link slot0 dataset
    df_link_slot0 = pd.read_csv(link_slot0_csv)
    df_link_slot0['sqrtPriceX96'] = pd.to_numeric(df_link_slot0['sqrtPriceX96'], errors='coerce')
    df_link_slot0['link_per_eth'] = df_link_slot0['sqrtPriceX96'].apply(sqrtPriceX96_to_price)
    ...
    # Rename 'blocknumber' to 'block_number' for link and ETH datasets
    df_link_slot0.rename(columns={'blocknumber': 'block_number'}, inplace=True)
    df_eth_slot0.rename(columns={'blocknumber': 'block_number'}, inplace=True)

    # Merge link price data
    df_merged = pd.merge(df_liquidations, df_link_slot0[['block_number', 'link_per_eth']],
    ...
    # Compute link price in USD
    df_merged['link_usd_price'] = df_merged['eth_usd_price'] * df_merged['link_per_eth']
...
def step_2_compute_baseline_bad_debt(df_merged: pd.DataFrame) -> pd.DataFrame:
    ...
    def _calc_baseline_bd(row):
        return compute_bad_debt_baseline(
            ...
            link_usd_price=row['link_usd_price']
        )
...
if __name__ == "__main__":
    # Example usage:
    liquidations_file = "/Users/hanson_zhang/PycharmProjects/svr_research/data/link_decrease_liqudiations.csv"
    link_slot0_file = "/Users/hanson_zhang/PycharmProjects/svr_research/data/LINK_slot0_21361217_21368389 copy.csv"
    eth_slot0_file = "/Users/hanson_zhang/PycharmProjects/svr_research/data/ETH_slot0_21361217_21368389 copy.csv"

    df_merged = step_1_load_and_merge(liquidations_file, link_slot0_file, eth_slot0_file)
    ...
    print(df_with_baseline[['evt_tx_hash', 'evt_block_number',
                            ...
                            'debtAsset_price', 'eth_usd_price', 'link_per_eth', 'link_usd_price',
...
# Progressive link price drops due to oracle delay
PROGRESSIVE_DROPS = {
    1: 0.0562,   # 5.62%
    2: 0.1124,   # 11.24% (5.62% + 5.62%)
    3: 0.1182,   # 11.82% (5.62% + 5.62% + 0.58%)
    4: 0.1276,   # 12.76% (5.62% + 5.62% + 0.58% + 0.94%)
    5: 0.1838    # 18.38% (5.62% + 5.62% + 0.58% + 0.94% +5.62%)
}
...
def step_3_model_oracle_delay(
    ...
    def _compute_delayed_bd(row, drop_fraction: float):
        """
        Compute bad debt for a single liquidation event given an link price drop.
        """
        # 1) Compute new link price under delayed scenario
        delayed_link_price = row['link_usd_price'] * (1.0 - drop_fraction)

        # 2) Compute new collateral value in USD under delayed price
        collateral_value_new = row['liquidated_collateral_amount'] * delayed_link_price
