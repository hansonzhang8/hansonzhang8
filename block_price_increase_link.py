...
def find_largest_increase(df, group_size):
    """
    Identify the largest final-step percentage increase in link/USD price.
    """
    ...
    for i in range(0, len(df_sorted) - group_size + 1, group_size):
        ...
        prices = subset['link_usd_price'].astype(float).tolist()
...
# Load Data
# ===============================
eth_link_file = "/Users/hanson_zhang/PycharmProjects/svr_research/data/LINK_slot0_21361217_21368389 copy.csv"
eth_usd_file = "/Users/hanson_zhang/PycharmProjects/svr_research/data/ETH_slot0_21361217_21368389 copy.csv"

df_eth_link = pd.read_csv(eth_link_file)
df_eth_usd = pd.read_csv(eth_usd_file)

# Convert sqrtPriceX96 to real price values
df_eth_link["sqrtPriceX96"] = pd.to_numeric(df_eth_link["sqrtPriceX96"], errors="coerce")  # Convert to float
df_eth_link["link_per_eth"] = df_eth_link["sqrtPriceX96"].apply(sqrt_price_to_token_ratio)
...
df_merged = pd.merge(df_eth_link, df_eth_usd, on="blocknumber", suffixes=("_link", "_usd"))
